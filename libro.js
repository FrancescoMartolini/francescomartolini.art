...
// OVERLAY PAGINE
// ════════════════════════════════
function apriPagina(tipo, { aggiornaURL = true } = {}) {
  const overlay = $('overlay-pagina');
  const contenuto = $('overlay-contenuto');
  contenuto.innerHTML = '';

  const infoSezione = SEZIONI_URL[tipo];
  if (aggiornaURL && infoSezione) {
    const url = `${BASE_PATH}/${infoSezione.slug}`;
    if (location.pathname !== url) {
      history.pushState({ pagina: tipo }, '', url);
    } else {
      history.replaceState({ pagina: tipo }, '', url);
    }
    document.title = `${infoSezione.titolo} — francescomartolini.art`;
  }
...
function chiudiPagina({ aggiornaURL = true } = {}) {
  $('overlay-pagina').classList.remove('aperta');
  if (aggiornaURL && leggiRoute()?.tipo === 'sezione') {
...
// ── Routing: URL parlanti (BASE/progetti/<id>, BASE/chi-sono, ecc.) ──
// L'URL cambia SOLO quando si apre/chiude una di queste pagine, mai
// durante lo sfogliare le pagine del libro (navigaA) o gli altri overlay
// minori: la navigazione interna resta "silenziosa" com'era prima, così
// mantiene la sensazione di libro. Serve solo per poter linkare/
// condividere una pagina precisa.
//
...
function chiudiProgetto({ aggiornaURL = true } = {}) {
  ...
  if (aggiornaURL && leggiRoute()?.tipo === 'progetto') {
    history.pushState({}, '', `${BASE_PATH}/`);
  }
  document.title = TITOLO_DEFAULT;
}

// ── Back/forward del browser: apre o chiude la pagina giusta senza
// toccare di nuovo la history (è già lì, ci stiamo solo muovendo dentro) ──
window.addEventListener('popstate', () => {
  const route = leggiRoute();
  // Chiudi tutto prima di aprire la destinazione giusta (si può arrivare
  // da un'altra pagina qualsiasi tramite indietro/avanti).
  chiudiProgetto({ aggiornaURL: false });
  chiudiPagina({ aggiornaURL: false });
  chiudiTaccuino({ aggiornaURL: false });

  if (!route) return; // torna alla home
  if (route.tipo === 'progetto') apriProgetto(route.id, { aggiornaURL: false });
  else if (route.tipo === 'taccuino') apriTaccuino({ aggiornaURL: false });
  else if (route.tipo === 'sezione') apriPagina(route.pagina, { aggiornaURL: false });
});

// ── Taccuino archivio ──
let _cacheTaccuino = null;

function apriTaccuino({ aggiornaURL = true } = {}) {
  const el = $('pagina-taccuino-archivio');
  const interno = el.querySelector('.taccuino-archivio-interno');

  if (aggiornaURL) {
    const url = `${BASE_PATH}/taccuino`;
    if (location.pathname !== url) {
      history.pushState({ taccuino: true }, '', url);
    } else {
      history.replaceState({ taccuino: true }, '', url);
    }
...
function chiudiTaccuino({ aggiornaURL = true } = {}) {
  $('pagina-taccuino-archivio').classList.remove('aperta');
  if (aggiornaURL && leggiRoute()?.tipo === 'taccuino') {
    history.pushState({}, '', `${BASE_PATH}/`);
  }
  document.title = TITOLO_DEFAULT;
}
...
async function init() {
  ...
  if (route?.tipo === 'progetto') {
    ...
    if (pr) {
      history.replaceState({ progetto: route.id }, '', `${BASE_PATH}/progetti/${route.id}`);
      apriProgetto(route.id, { aggiornaURL: false });
    } else {
      history.replaceState({}, '', `${BASE_PATH}/`);
    }
  } else if (route?.tipo === 'taccuino') {
    history.replaceState({ taccuino: true }, '', `${BASE_PATH}/taccuino`);
    apriTaccuino({ aggiornaURL: false });
  } else if (route?.tipo === 'sezione') {
    history.replaceState({ pagina: route.pagina }, '', `${BASE_PATH}/${SEZIONI_URL[route.pagina].slug}`);
    apriPagina(route.pagina, { aggiornaURL: false });
  }
}
